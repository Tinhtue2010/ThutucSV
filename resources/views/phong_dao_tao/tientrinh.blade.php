    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ __('Tiến trình sử lý đơn ') }}</h5>
...
@push('js')
    <script type="text/javascript">
        ...
        function tientrinh(data) {
            ...
            id{{ $target }} = data;
            $('.list_data').html('');
            axios.get("{{ route('PhongDaoTao.getDataChild') }}/" + data).then(
                response => {
                    model{{ $target }}.show();
                    response.data[1].forEach(item => {
                        ...
                        <div class="d-flex flex-column">
                            ...
                            <p class="fs-5 fw-medium">Trạng thái : <span class="badge badge-${item.status == 1 ? 'success' : item.status == 0 ? 'danger' : 'warning'}">${item.status == 1 ? 'Đã xác nhận' : item.status == 0 ? 'Đã từ chối' : 'Yêu cầu bổ xung hồ sơ'}</span></p>

    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tạo quyết định trợ cấp xã hội</h5>
...
@push('js')
    <script type="text/javascript">
        ...
        $('#form_{{ $target }}').submit(function(e) {
            ...
            validation_{{ $target }}.validate().then(function(status) {
                ...
                if (status === 'Valid') {
                    axios({
                        method: 'POST',
                        url: "{{ route('PhongDaoTao.TroCapXaHoi.createQuyetDinh') }}",
                        data: form.serialize(),
                    }).then((response) => {
        ...
        function taoQuyetDinhMGHP() {
            ...
            axios.get("{{ route('PhongDaoTao.TroCapXaHoi.getQuyetDinh') }}").then(
                response => {

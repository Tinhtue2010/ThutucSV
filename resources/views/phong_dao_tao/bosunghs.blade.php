    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ __('Yêu cầu bổ xung hồ sơ') }}</h5>
                ...
                <form class="form" id="form_{{$target}}">
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-column mb-8 fv-row">
                            ...
                            <!--end::Label-->
                            <textarea type="text" class="form-control form-control-solid" cols="5" rows="3" name="note">Đơn của bạn đã bị từ chối bởi phòng đào tạo</textarea>
                        </div>
        ...
        $('#form_{{$target}}').submit(function (e) {
            ...
            validation_{{$target}}.validate().then(function (status) {
                if (status === 'Valid') {
                    axios({
                        method: 'POST',
                        url: "{{ route('PhongDaoTao.khongxacnhan') }}",
                        data: form.serialize(),
                    }).then((response) => {

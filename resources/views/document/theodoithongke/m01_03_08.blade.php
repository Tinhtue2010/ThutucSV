...
@section('data')
    <div id="doc_view" class="A4 d-flex flex-column" style="height: auto;">
        ...
        <div class="text-center fw-bold" style="margin-top: 35px; text-transform: uppercase">
            DANH SÁCH SINH VIÊN ĐƯỢC HỖ TRỢ CHI PHÍ HỌC TẬP <br />
            HỌC KỲ I NĂM HỌC 2022 – 2023
        </div>
        <div class="fst-italic text-center mx-auto" style="max-width: 85%">
            (Kèm theo quyết định số {{$data[0]['so_QD']}}/QĐ-ĐHHL, ngày {{$data[0]['thoi_gian_tao_ngay']}} tháng {{$data[0]['thoi_gian_tao_thang']}} năm {{$data[0]['thoi_gian_tao_nam']}} của Hiệu
...
@endforeach
                ...
                <tr style="height: 30px">
                    <td class="" colspan="4">Tổng: {{$data[0]['tong']}}</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
        ...
        <div class="d-flex flex-row justify-content-between px-5 mt-1">
            <div class="d-flex justify-content-end text-center fw-bold align-items-baseline">
                <span>số tiền bằng chữ: </span><span class="fw-light ms-1"> {{ numberInVietnameseCurrency($data[0]['tong']) }}</span>
            </div>

...
class LanhDaoPhongDaoTaoController extends Controller
{
    function index()
    {
        ...
        return view('lanh_dao_phong_dao_tao.index');
    }
    ...
    function xacnhan(Request $request)
    {
        try {
            ...
            if ($stopStudy->status != 6 && $stopStudy->status != -7 && $stopStudy->status != 7) {
                abort(404);
            }
    ...
    function tuchoihs(Request $request)
    {
        ...
        if ($request->button_clicked == "huy_phieu" && $stopStudy->status == -7) {
            $newStopStudy = $stopStudy->where('parent_id', $request->id)->orderBy('created_at', 'desc')->first();
            if ($newStopStudy) {
                try {
                    $phieu = Phieu::find($newStopStudy->phieu_id);
